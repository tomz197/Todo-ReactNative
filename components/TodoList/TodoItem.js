...
export default function TodoItem(props) {
  ...
  const deleteButton = 
    [<TouchableOpacity style={[styles.todoButton, styles.deleteButton]} onPress={() => handleDelete(item._id)}>
      <MaterialIcons name="delete" size={30} color="black" />
    </TouchableOpacity>]

  return (
    <View
        style={[styles.todoContainer, itemClass]}
        key={item._id}
      >
        <Swipeable rightButtons={deleteButton} rightButtonWidth={100}>
          <View style={{flexDirection: 'row'}}>
            ...
            <View style={styles.todoButtonsContainer}>
              {item.state !== 2
              && <TouchableOpacity style={styles.todoButton} onPress={() => handleUpdate(item._id, 2)}>
                <MaterialIcons name="done" size={25} color="black" />
              </TouchableOpacity>}
              {item.state !== 1
              && <TouchableOpacity style={styles.todoButton} onPress={() => handleUpdate(item._id, 1)}>
                <MaterialCommunityIcons name="progress-check" size={25} color="black" />
              </TouchableOpacity>}
              {item.state !== 0
              && <TouchableOpacity style={styles.todoButton} onPress={() => handleUpdate(item._id, 0)}>
